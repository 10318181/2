...
try:
    ...
    if n4>len(dna): #Criei uma condicional com if
        ...
        if cds1 != '' and cds2 != '':
            string_final = cds1 + cds2 #Jundo os dois fragmentos de DNA que obedecem as minhas condições
            print(string_final) #Printo as duas sequencias juntas (minha sequencia final)
        
except ValueError:
    print("Um dos valores de 'n' inseridos não é um número inteiro") #Adiciono mais uma condição para que o programa avise ao usuario se o valor inserido para n não é numerio. O valor de n precisa ser numerico
